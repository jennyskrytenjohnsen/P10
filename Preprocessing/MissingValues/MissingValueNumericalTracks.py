    ...
    for index, row in df_tracklist.iterrows(): # For loop which runs over every row in the df_trackist
        if row['tname'] in special_variables: #Evaluates if the name in the row a special variable
            ...
            if how_many_samples_should_there_be > 1.2 * total_length_of_singal:
                print('No index: ', index," of ",index_len, 'There is', total_length_of_singal, 'samples', 
                ...
                print('Yes index: ', index," of ",index_len, 'There is', total_length_of_singal, 'samples', 'but there should be', how_many_samples_should_there_be, 'samples', 'This precentage of the signal is precent', precentage_of_signal_is_there, '%')
